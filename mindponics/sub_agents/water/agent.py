...
def diagnose_water_quality(parameters: dict) -> dict:
    ...
    return {
        ...
        "issues": issues,
        "status": "optimal" if not issues else "issues_detected"
    }
...
def suggest_corrective_actions(parameters: dict, diagnosis: dict) -> dict:
    ...
    for issue in diagnosis.get("issues", []):
        if issue["severity"] == "critical":
            if "ammonia" in issue["parameter"]:
                actions.append("Perform immediate 25-50% water change")
                actions.append("Reduce feeding immediately")
                actions.append("Check biofilter function")
            elif "nitrite" in issue["parameter"]:
                actions.append("Perform immediate 25-50% water change")
                actions.append("Add salt (1-3 ppt) to protect fish")
                actions.append("Check biofilter function")
            elif "oxygen" in issue["parameter"]:
                actions.append("Increase aeration immediately")
                actions.append("Reduce stocking density if possible")
                actions.append("Add additional air stones or surface agitation")
    ...
    if params["ammonia"] > 0.5:
        ...
        actions.append("Add beneficial bacteria supplement")
    
    if params["nitrate"] > 150:
        actions.append("Perform 20% water change")
        actions.append("Increase plant density to consume more nitrates")
    
    if params["temperature"] > 30.0:
        actions.append("Install water chiller or shade system")
        actions.append("Increase aeration as warm water holds less oxygen")
...
WaterQualityDiagnosisTool = FunctionTool(
    ...
    function=diagnose_water_quality
)
...
CorrectiveActionSuggesterTool = FunctionTool(
    ...
    function=suggest_corrective_actions
)
